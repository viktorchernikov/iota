...
public class CuttingWorker
{
    public CuttingWorkerState state { get; private set; } = CuttingWorkerState.None;
    public bool isOccupied { get; private set; } = false;


    private CuttableObject targetObject;
    ...
    public void Occupy(CuttableObject target, Vector3 contactPoint, Vector3 cutNormal)
    {
        if (isOccupied)
        {
            throw new UnityException("Cutting worker was already busy, but somebody tried to procced new cutting anyway!");
        }

        isOccupied = true;

        targetObject = target;
        originalMesh = targetObject.GetMesh();
        originalMeshVertices = originalMesh.vertices;
        originalMeshNormals = originalMesh.normals;
        originalMeshUv = originalMesh.uv;
        cutPlane = new Plane(targetObject.transform.InverseTransformDirection(-cutNormal), targetObject.transform.InverseTransformPoint(contactPoint));
    ...
    public void Proceed()
    {
        if (!isOccupied)
        {
            throw new UnityException("Cutting worker was not set up yet!");
        }
        ...
        targetObject.SetColision(finishedLeftMesh);
        targetObject.UpdateMesh(finishedLeftMesh);

        int matsCount = Mathf.Min(finishedLeftMesh.subMeshCount, targetObject.GetRenderer().materials.Length);
        Material[] mats = new Material[matsCount];
        for (int i = 0; i < matsCount; i++)
        {
            mats[i] = targetObject.GetRenderer().materials[i];
        }
        targetObject.GetRenderer().materials = mats;

        GameObject right = new GameObject();

        right.transform.position = targetObject.transform.position + (Vector3.up * .05f);
        right.transform.rotation = targetObject.transform.rotation;
        right.transform.localScale = targetObject.transform.localScale;

        matsCount = Mathf.Min(finishedRightMesh.subMeshCount, targetObject.GetRenderer().materials.Length);
        ...
        var leftRigidbody = targetObject.GetComponent<Rigidbody>();
    ...
    public void Release()
    {
        ...
        targetObject = null;
        cutPlane = default;
        originalMeshVertices = null;
        originalMeshNormals = null;
        originalMeshUv = null;
        leftMesh.Clear();
        rightMesh.Clear();
    }
...
public enum CuttingWorkerState
{
    None,
    Ready,
    Proceeding,
